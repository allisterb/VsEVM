...
namespace VsEVM
{
    [ExportFileContextActionProvider((FileContextActionProviderOptions)VsCommandActionProviderOptions.SupportVsCommands, ProviderType, ProviderPriority.Normal, VsEVMPackageIds.SolidityFileContextType)]
    public class SolidityFileContextActionProviderFactory : IWorkspaceProviderFactory<IFileContextActionProvider>, IVsCommandActionProvider
    {
        ...
        private static readonly Guid ProviderCommandGroup = VsEVMPackageIds.GuidStratisEVMPackageCmdSet;
        private static readonly IReadOnlyList<CommandID> SupportedCommands = new List<CommandID>
        {
            new CommandID(VsEVMPackageIds.GuidStratisEVMPackageCmdSet, VsEVMPackageIds.Cmd1Id),
            new CommandID(VsEVMPackageIds.GuidStratisEVMPackageCmdSet, VsEVMPackageIds.Cmd4Id),
        };
            ...
            public Task<IReadOnlyList<IFileContextAction>> GetActionsAsync(string filePath, FileContext fileContext, CancellationToken cancellationToken)
            {
                return Task.FromResult<IReadOnlyList<IFileContextAction>>(new IFileContextAction[]
                {
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(ProviderCommandGroup, VsEVMPackageIds.Cmd1Id),
                    ...
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(VsEVMPackageIds.GuidStratisEVMPackageCmdSet, VsEVMPackageIds.Cmd4Id),
                        ...
                        async (fCtxt, progress, ct) =>
                        {
                            ...
                            if (r != null)
                            {
                                var window = (SolidityStaticAnalysisToolWindow) await VsEVMPackage.Instance.ShowToolWindowAsync(typeof(SolidityStaticAnalysisToolWindow), 0, true, VsEVMPackage.Instance.DisposalToken);
